...
EXPORT_OPTIONS_PLIST = "exportOptions_release.plist"
#会在桌面创建输出ipa文件的目录
...
def buildExportDirectory(scheme):
	...
	path = buildExportPath(scheme)
	exportDirectory = path#”%s%s%s” %(path, scheme, stdoutdata.strip())
...
def buildExportPath(tempName):
    process = subprocess.Popen("pwd", stdout=subprocess.PIPE)
    (stdoutdata, stderrdata) = process.communicate()
    archiveName = "%s.xcarchive" %(tempName)
    archivePath = stdoutdata.strip() + '/build/'
    return archivePath
